...
export function DashboardOverviewSkeleton() {
  return (
    <div className="flex flex-col gap-6 animate-pulse">
      <div className="bg-blue-600/70 p-5 rounded-2xl shadow-sm flex flex-col gap-3">
        <div className="flex items-center gap-2">
          <Skeleton className="w-5 h-5 rounded-full bg-white/30" />
          <Skeleton className="h-4 w-24 rounded bg-white/30" />
        </div>
        <Skeleton className="h-9 w-48 rounded-lg bg-white/40" />
      </div>

      <div>
        <Skeleton className="h-5 w-28 rounded mb-3" />
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-col gap-2">
            <Skeleton className="h-3 w-16 rounded" />
...
export function IngestionListSkeleton({ count = 4 }: { count?: number }) {
  return (
    <div className="p-4 flex flex-col gap-3 max-w-4xl mx-auto w-full">
      {Array.from({ length: count }).map((_, i) => (
        <div
          key={i}
          className="p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-col gap-3 shadow-sm animate-pulse"
        >
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2">
